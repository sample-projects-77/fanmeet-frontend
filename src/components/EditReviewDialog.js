...
export function EditReviewDialog({ review, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await reviewAPI.updateReview(review.id, { rating, comment: comment.trim() });
  ...
  return (
    <div className="give-review-dialog-backdrop" onClick={handleBackdropClick} role="dialog" aria-modal="true" aria-labelledby="edit-review-title">
      <div className="give-review-dialog">
        <h2 id="edit-review-title" className="give-review-dialog-title">{t('reviews.editReview')}</h2>
        ...
        <form onSubmit={handleSubmit} className="give-review-form">
          ...
          <label className="give-review-label" htmlFor="edit-review-comment">{t('reviews.comment')}</label>
          <textarea
